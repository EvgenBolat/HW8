...
void sigfunc(int sig)
{
  ...
  {
    printf("sem_close: Incorrect close of reader semaphore");
    exit(-1);
  };
  // Удаляет свой семафор
  if (sem_unlink(reader_sem_name) == -1)
  {
    printf("sem_unlink: Incorrect unlink of reader semaphore");
    // exit(-1);
  };
  printf("Reader: bye!!!\n");
  exit(10);
...
int main()
{
  ...
  if (sem_wait(admin) == -1)
  { // ожидание когда запустится писатель
    printf("sem_wait: Incorrect wait of admin semaphore");
    exit(-1);
  };
  ...
  if (sem_post(admin) == -1)
  {
    printf("sem_post: Consumer can not increment admin semaphore");
    exit(-1);
  }
  ...
  {
    printf("shm_open: Incorrect reader access");
    exit(-1);
  }
  else
  {
  ...
  {
    printf("sem_open: Can not create reader semaphore");
    exit(-1);
  };
  // Первый просочившийся запрещает доступ остальным за счет установки флага
  if (sem_wait(reader) == -1)
  { // ожидание когда запустится писатель
    printf("sem_wait: Incorrect wait of reader semaphore");
    exit(-1);
  };
  ...
  if (buffer->have_reader)
  {
    ...
    {
      printf("sem_post: Consumer can not increment reader semaphore");
      exit(-1);
    }
    exit(13);
  }
  ...
  while (1)
  {
    ...
    if (sem_wait(full) == -1)
    {
      printf("sem_wait: Incorrect wait of full semaphore");
      exit(-1);
    };
    // критическая секция
    if (sem_wait(mutex) == -1)
    {
      printf("sem_wait: Incorrect wait of busy semaphore");
      exit(-1);
    };
    if (ind == BUF_SIZE)
    {
    ...
    if (sem_post(empty) == -1)
    {
      printf("sem_post: Incorrect post of free semaphore");
      exit(-1);
    };
