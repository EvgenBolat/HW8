...
void sigfunc(int sig) {
  ...
  if(sem_close(writer) == -1) {
    printf("sem_close: Incorrect close of writer semaphore");
    exit(-1);
  };
  if(sem_close(first_writer) == -1) {
    printf("sem_close: Incorrect close of first_writer semaphore");
    exit(-1);
  };
  ...
  if(sem_unlink(writer_sem_name) == -1) {
    printf("sem_unlink: Incorrect unlink of writer semaphore");
    // exit(-1);
  };
  if(sem_unlink(first_writer_sem_name) == -1) {
    printf("sem_unlink: Incorrect unlink of first_writer semaphore");
    // exit(-1);
  };
  unlink_all();
  printf("Writer: bye!!!\n");
...
int main() {
 ...
 if ( (buf_id = shm_open(shar_object, O_CREAT|O_RDWR, 0666)) == -1 ) {
  ...
  if((writer = sem_open(writer_sem_name, O_CREAT, 0666, 1)) == 0) {
    printf("sem_open: Can not create writer semaphore");
    exit(-1);
  };
  ...
  if((first_writer = sem_open(first_writer_sem_name, O_CREAT, 0666, 1)) == 0) {
    printf("sem_open: Can not create first_writer semaphore");
    exit(-1);
  };
  // Первый просочившийся запрещает доступ остальным за счет установки флага
  if(sem_wait(writer) == -1) {
    printf("sem_wait: Incorrect wait of writer semaphore");
    exit(-1);
  };
  ...
  if(writer_number == 0) {
    ...
    if(sem_post(writer) == -1) {
      printf("sem_post: Consumer can not increment writer semaphore");
      exit(-1);
    }
    exit(13);
  }
  ...
  if(sem_wait(first_writer) == -1) {
    printf("sem_wait: Incorrect wait of first_writer semaphore");
    exit(-1);
  };
  ...
  if(is_writers == 0) {
    ...
    if(sem_post(admin) == -1) {
      printf("sem_post: Can not increment admin semaphore");
      exit(-1);
    }
  }
  ...
  while (1) {
    ...
    if(sem_wait(empty) == -1) { //защита операции записи
      printf("sem_wait: Incorrect wait of empty semaphore");
      exit(-1);
    };
    //критическая секция, конкуренция с читателем
    if(sem_wait(mutex) == -1) {
      printf("sem_wait: Incorrect wait of mutex");
      exit(-1);
    };
    ...
    if(sem_post(full) == -1) {
      printf("sem_post: Incorrect post of full semaphore");
      exit(-1);
    };
    ...
    if(sem_post(mutex) == -1) {
      printf("sem_post: Incorrect post of mutex semaphore");
      exit(-1);
    };
    sleep(rand() % 3 + 1);
  }
